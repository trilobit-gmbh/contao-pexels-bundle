...
class PexelsApi
{
    ...
    {
        //$strApiUrl = Config::get('pexelsApiUrl');
        $strApiUrl = Helper::getConfigData()['api_url'];
        $strApiKey = Config::get('pexelsApiKey');
        ...
        $strCacheFile = $strCachePath.'/contao/pexels/'.$strChecksum.'.json';
        ...
        if (null === $arrResult) {
            try {
                ...
                System::log('Pexels search failed: '.$e->getMessage(), __METHOD__, TL_ERROR);
                $arrResult = [];
        ...
        $arrResult['totalHits'] = $arrResult['total_results'];
        $arrResult['resultsPerPage'] = $arrResult['per_page'];
        $arrResult['pexelsPage'] = $arrResult['page'];
    ...
    {
        $strUrl = $strApiUrl.'?'.http_build_query($arrApiParameter);

        $objCurl = curl_init();

        curl_setopt($objCurl, CURLOPT_URL, $strUrl);

        curl_setopt($objCurl, CURLOPT_USERAGENT, 'Contao Pexels API');
        curl_setopt($objCurl, CURLOPT_HTTPHEADER, ['Authorization: '.$strApiKey]);
